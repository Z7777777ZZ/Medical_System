...
```
os, flask_sqlalchemy, re, requests, json
```

## 页面跳转逻辑

左侧边栏是两个子模块入口，即智慧问诊入口和医生查找入口。

智慧问诊模块中，有一个输入框，在输入框中输入症状后，点击提交症状按钮后，等待后端返回结果产生AI诊断结果和推荐医生，推荐医生卡片中有详情按钮和预约按钮。点击详情按钮后出现医生详情页，包含姓名、医院、科室、专长、电话、个人简介、平均评分、评论等信息，详情页中还有添加评论入口，选择评分和填写评论（可选）后可以提交评论。**预约按钮可以接入其他组中的预约挂号模块，目前预约按钮接入的是前端模拟的页面。**

医生查找模块中，可以输入关键词（如姓名、医院、科室、专长，输入框为空表示全匹配），也可以附加选择医院、科室、排序方式、排序顺序等条件进行筛选，点击查询后会显示符合条件的医生的卡片。医生卡片上有姓名、电话、医院、科室、专长、评分等信息，还有详情按钮和预约按钮。和智慧问诊模块推荐医生卡片类似，点击详情按钮后出现医生详情页，包含姓名、医院、科室、专长、电话、个人简介、平均评分、评论等信息，详情页中还有添加评论入口，选择评分和填写评论（可选）后可以提交评论。**预约按钮可以接入其他组中的预约挂号模块，目前预约按钮接入的是前端模拟的页面。**
